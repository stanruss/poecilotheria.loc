...
$lang = array_merge($lang, array(
	...
	'BOARD_INDEX_TEXT'					=> 'Текст, указанный в списке форумов',
	'BOARD_INDEX_TEXT_EXPLAIN'			=> 'Этот текст отображается в хлебных крошках. Если ничего не указано, то по умолчанию отображается список форумов.',
	...
	'DISABLE_BOARD_EXPLAIN'				=> 'Форум станет недоступен для пользователей, не являющихся администраторами или модераторами. Также вы можете ввести короткое сообщение (до 255 символов), которое будут видеть посетители.',
	'DISPLAY_LAST_SUBJECT'				=> 'Показывает тему последнего сообщения в списке тем форумов',
	'DISPLAY_LAST_SUBJECT_EXPLAIN'		=> 'Тема последнего добавленного сообщения будет показана в списке тем форумов с гиперссылкой на сообщение. Темы из форумов, защищённых паролем, и форумов, к которым пользователь не имеет доступа, выводится не будут.',
	...
	'OVERRIDE_STYLE_EXPLAIN'			=> 'Стиль, выбранный пользователем (или гостем), будет заменен стилем, указанным в настройке «Стиль по умолчанию».',
	'SITE_DESC'							=> 'Описание сайта',
	'SITE_HOME_TEXT'					=> 'Главный текст сайта',
	'SITE_HOME_TEXT_EXPLAIN'			=> 'Этот текст будет отображаться в виде ссылки на домашнюю страницу Вашего сайта в хлебных крошках форума. Если не определено, то по умолчанию будет отображаться домашняя страница.',
	'SITE_HOME_URL'						=> 'Основной URL веб-сайта',
	'SITE_HOME_URL_EXPLAIN'				=> 'Если указано, то ссылка на эту страницу будет отображена в хлебных крошках и логотип форума будет ссылкой на эту страницу вместо главной страницы форума. Рекомендуем установить абсолютную, например <samp>http://www.phpbb.com</samp>.',
...
$lang = array_merge($lang, array(
	...
	'ALLOW_GRAVATAR'					=> 'Включить глобально распознаваемые аватары',
...
$lang = array_merge($lang, array(
	...
	'ACC_ACTIVATION_EXPLAIN'		=> 'Определить, должен ли пользователь получить немедленный доступ к форуму или для этого требуется подтверждение регистрации. Вы можете также полностью отключить регистрацию новых пользователей. <em>Функция «Разрешить email-сообщения» должна быть включена для использования возможности активации новых учётных записей пользователем или администратором.</em>',
...
$lang = array_merge($lang, array(
	...
	'ALLOW_CDN'						=> 'Разрешить использование сторонних сетей доставки контента',
	...
	'LOAD_CPF_PM'						=> 'Показывать пользовательский профиль поля в личных сообщениях',
	...
	'READ_NOTIFICATION_EXPIRE_DAYS'		=> 'Узнавать окончание срока действия уведомлений',
	'READ_NOTIFICATION_EXPIRE_DAYS_EXPLAIN' => 'Количество дней, которое должно пройти до уведомления о прочтении, перед автоматическим удалением. Установите значение 0, чтобы сделать уведомления постоянным.',
...
$lang = array_merge($lang, array(
	...
	'AUTH_PROVIDER_OAUTH_ERROR_ELEMENT_MISSING'	=> 'Должен быть обеспечен ключ и секрет каждого включенного поставщика OAuth услуг. Только один может быть предоставлен для поставщика OAuth услуг.',
	...
	'APACHE_SETUP_BEFORE_USE'			=> 'Вы должны настроить аутентификацию apache при переключении phpBB на этот метод. Помните, что имя пользователя для аутентификации в Apache должно совпадать с Вашим именем пользователя в phpBB. Аутентификация apache может быть использована только с mod_php (не с a CGI версией) и отключенным safe_mode.',
...
$lang = array_merge($lang, array(
	...
	'MOD_REWRITE_ENABLE'			=> 'Включить перезапись URL',
	'MOD_REWRITE_ENABLE_EXPLAIN' 	=> 'При включении, URL содержащие вЂ™app.phpвЂ™ будут переписаны для удаления имени файла (то есть app.php/foo станут /foo). <strong>Apache serverвЂ™s требуется модуль mod_rewrite для того, чтобы эта возможность работала; если эта опция будет активирована при отключенной поддержке mod_rewrite, URL на форуме могуть быть повреждены.</strong>',
	'MOD_REWRITE_DISABLED'			=> 'Модуль <strong>Mod_rewrite</strong> на Вашем Apache веб-сервер отключен. Включите модуль или свяжитесь со своим хостинг-провайдером, если Вы хотите включить эту функцию.',
	'MOD_REWRITE_INFORMATION_UNAVAILABLE' => 'Мы не в состоянии определить, поддерживает ли данный сервер перезапись URL. Этот параметр может быть включен, но если URL не доступны для перезаписи, ссылки, сгенерированные форумом, могут быть повреждены. Свяжитесь с Вашим хостинг провайдером, если Вы не уверены,  безопасно ли включение этой функции.',
	...
	'USE_SYSTEM_CRON'				=> 'Запустить периодические задачи системы cron',
...
$lang = array_merge($lang, array(
	...
	'ALLOW_PASSWORD_RESET'				=> 'Разрешить сброс пароля ("Забытый пароль")',
	'ALLOW_PASSWORD_RESET_EXPLAIN'		=> 'Определяет, могут ли пользователи использовать ссылку "Я забыл мой пароль" на странице входа в систему, чтобы восстановить учетную запись. Если Вы используете внешний механизм проверки подлинности, возможно, Вы захотите отключить данную функцию.',
	'AUTOLOGIN_LENGTH'					=> 'Продолжительность действия функции "Запомнить меня" (введите количество дней)',
	'AUTOLOGIN_LENGTH_EXPLAIN'			=> 'Укажите количество дней, по истечении которых функция "Запомнить меня" обнуляется или ноль, чтобы отключить.',
	...
	'REFERRER_VALID_EXPLAIN'			=> 'Если включено, рефёрер для запросов типа POST будет проверен на параметры хоста/пути к скрипту. Это может вызвать проблемы с форумами, использующими несколько доменов или внешние аутентификации.',
...
$lang = array_merge($lang, array(
	...
	'ENABLE_EMAIL_EXPLAIN'				=> 'Если выключено, отправка любых e-mail сообщений с форума производится не будет. <em>Примечание: данная опция должна быть включена, если вы хотите задействовать активацию аккаунта администратором или пользователем. Если в настоящий момент выбран метод активации новых аккаунтов «Администратором» или «Пользователем», отключение данной опции сделает недоступной регистрацию новых пользователей.</em>',
